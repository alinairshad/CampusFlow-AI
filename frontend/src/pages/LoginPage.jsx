...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(typeof detail === 'string' ? detail : Array.isArray(detail) ? detail.map(d => d.msg).join(', ') : 'Login failed. Please try again.')
    } finally {
      setLoading(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" name="email" value={form.email} onChange={handleChange} required
                   className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-lgu-400"
