...
function AssistantBubble({ content, type, sources, found }) {
  return (
    <div className="flex items-end gap-2 mb-4">
      {/* Avatar */}
      ...
      <div className="max-w-[78%] space-y-1.5">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl rounded-bl-sm px-4 py-2.5 shadow-sm">
          {/* Type badge — only for problem/application */}
          {(type === 'problem' || type === 'application') && (
            <span className={`inline-block text-xs font-medium px-2 py-0.5 rounded-full mb-1.5 ${
              type === 'problem'
                ? 'bg-orange-100 text-orange-700'
                : 'bg-purple-100 text-purple-700'
            }`}>
              {type === 'problem' ? 'Problem' : 'Application'}
            </span>
          )}
        ...
        {/* Source chips — shown only when found=True */}
        {found === true && sources.length > 0 && (
